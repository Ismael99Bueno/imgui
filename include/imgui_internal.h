...
#define IM_MSVC_WARNING_SUPPRESS(XXXX) __pragma(warning(suppress \
                                                        : XXXX))
#else
...
struct ImPool
{
    ...
    {
        return GetMapSize();
    } // For ImPlot: should use GetMapSize() from (IMGUI_VERSION_NUM >= 18304)
#endif
};
...
struct ImGuiKeyRoutingData
{
    ...
    ImGuiKeyRoutingData()
    {
        ...
        RoutingCurr = RoutingNext = ImGuiKeyOwner_None;
    }
};
...
struct ImGuiKeyOwnerData
{
    ...
    ImGuiKeyOwnerData()
    {
        OwnerCurr = OwnerNext = ImGuiKeyOwner_None;
